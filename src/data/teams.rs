...
use crate::model::team::{Team, TeamConfig};
...
pub fn load_teams(claude_home: &Path, project_cwd: Option<&Path>) -> Result<Vec<Team>> {
    ...
    for entry in entries.flatten() {
        ...
        if !config_path.exists() {
            // Check for inboxes directory — UUID-based team without config
        ...
        if let Some(cwd) = project_cwd {
            ...
            let matches = config.members.iter().any(|m| {
                if let Some(ref member_cwd) = m.cwd {
                    ...
                    // Component-aware: exact match or is a parent
    ...
    // If no team with config matched the CWD, load all teams
    // (UUID teams without config don't count as a CWD match)
